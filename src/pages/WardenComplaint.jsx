...
const WardenComplaints = () => {
  ...
  const handleApproval = async (id) => {
    ...
    try {
      const response = await fetch(`https://arjun-college-project-backend.onrender.com/complaints/${id}`, {
        method: "POST",
        headers: GetAuthHeader(),
      });
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-3 ml-4 mr-4">
  ...
  {complaint.assigned_at ? (
    `Completed on ${formatTimestamp(complaint.assigned_at)}`
      ...
      <div className="bg-red-100 p-4 sm:p-8 md:p-10 h-screen">
        ...
        {complaints.length === 0 ? (
          ...
          <div className="container mx-auto grid gap-8 md:grid-cols-3  sm:grid-cols-1">
            {complaints.map((complaint) => (
              <div key={complaint.complaint_id} className="relative flex h-full flex-col rounded-md border border-gray-200 bg-orange-200 p-2.5 hover:border-gray-400 sm:rounded-lg sm:p-5">
